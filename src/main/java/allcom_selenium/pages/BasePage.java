...
public class BasePage {
  ...
  public BasePage(WebDriver driver) {
    this.driver = driver;
    this.wait = new FluentWait<>(driver).withTimeout(Duration.ofMillis(100)).pollingEvery(Duration.ofMillis(100)).ignoring(NoSuchElementException.class);
    PageFactory.initElements(driver, this);
  }

  public void waitUntilUrlToBe(String url) {
    final int MAX_ATTEMPTS = 2; // Set the number of attempts here
    int attempts = 0;
    ...
    while (attempts < MAX_ATTEMPTS) {
      try {
        ...
        logger.info("Waiting for URL to be [" + expectedUrl + "], Time taken: [" + timeTakenMillis + "] milliseconds, Attempts: [" + MAX_ATTEMPTS + "]");
        break;
  ...
  public void click(WebElement element) {
    ...
    try {
      ...
      long timeTakenMillis = Duration.between(start, Instant.now()).toMillis();
      logger.info("Clicked on Element [{}], Time taken: [{}] milliseconds", elementDescription, timeTakenMillis);
    } catch (ElementNotInteractableException e) {
      throw new RuntimeException("Element is not interactable: [" + elementDescription + "]", e);
    } catch (StaleElementReferenceException e) {
      ...
      throw new RuntimeException("WebDriver exception: [" + elementDescription + "]", e);
    }
  }
  ...
  public void clickJSExecutor(WebElement element) {
    ...
    try {
      ...
      long timeTakenMillis = Duration.between(start, Instant.now()).toMillis();
      logger.info("Click on Element [{}], Time taken: [{}] milliseconds", elementDescription, timeTakenMillis);
    } catch (ElementNotInteractableException e) {
      throw new RuntimeException("Element is not interactable: [" + elementDescription + "]", e);
    } catch (StaleElementReferenceException e) {
      throw new RuntimeException("Stale element reference: [" + elementDescription + "]", e);
  ...
  public void type(WebElement element, String text) {
    ...
    if (text != null) {
      try {
        ...
        element.clear();
        logger.info("Clear Element [{}]", elementDescription);
        element.sendKeys(text);
        logger.info("Type [{}] to [{}]", text, elementDescription);
      } catch (ElementNotInteractableException e) {
        throw new RuntimeException("Element is not intractable: [" + element + "]", e);
      } catch (StaleElementReferenceException e) {
  ...
  public WebElement waitForElement(By locator, int timeoutInSeconds) {
    return new WebDriverWait(driver, Duration.ofSeconds(timeoutInSeconds)).ignoring(StaleElementReferenceException.class).until(ExpectedConditions.visibilityOfElementLocated(locator));
  }
  ...
  public void waitForElementToAppear(ElementType type, String value, boolean expectedStatus, int timeoutInSeconds) {
    ...
    try {
      ...
      logger.info("Element can't be found on the page in " + timeoutInSeconds + " seconds");
  ...
  public void isElementClickable(By locator, Dimension resolution, boolean expectedStatus) {
    ...
    boolean isClickable;
    try {
      wait.withTimeout(WAIT_SEC).pollingEvery(Duration.ofMillis(100)).ignoring(NoSuchElementException.class).until(ExpectedConditions.elementToBeClickable(locator));
      isClickable = true;
    } catch (TimeoutException e) {
      isClickable = false;
      logger.error("TimeoutException: Element was not clickable within the specified wait time [{}] seconds", WAIT_SEC.getSeconds());
    } catch (NoSuchElementException e) {
      isClickable = false;
      logger.error("NoSuchElementException: Element [{}] was not found.", locator.toString());
    }
